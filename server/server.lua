...
local cooldowns = {}
Core.Callback.Register('bcc-waves-itskaaas:CheckCooldown', function(source, cb, location)
    local src = source
...
RegisterServerEvent('bcc-waves-itskaaas:RewardPayout', function(lootCfg)
    local src = source
    local user = Core.getUser(src)
...
Core.Callback.Register('bcc-waves-itskaaas:RobberyCheck', function(source, cb)
    local src = source
    local user = Core.getUser(src)
    if not user then return cb(false) end
    local character = user.getUsedCharacter
...
RegisterServerEvent('bcc-waves-itskaaas:DeletePed', function(netId)
